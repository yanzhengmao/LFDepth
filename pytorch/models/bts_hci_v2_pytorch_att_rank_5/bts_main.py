...
parser.add_argument('--max_depth',                 type=float, help='maximum depth in estimation', default=1.55)
parser.add_argument('--min_depth',                 type=float, help='minimum depth in estimation', default=-0.47)

# Log and save
parser.add_argument('--log_directory',             type=str,   help='directory to save checkpoints and summaries', default='')
parser.add_argument('--checkpoint_path',           type=str,   help='path to a checkpoint to load', default='')
parser.add_argument('--log_freq',                  type=int,   help='Logging frequency in global steps', default=170)
...
parser.add_argument('--min_depth_eval',            type=float, help='minimum depth for evaluation', default=-0.47)
parser.add_argument('--max_depth_eval',            type=float, help='maximum depth for evaluation', default=1.55)
parser.add_argument('--eigen_crop',                            help='if set, crops according to Eigen NIPS14', action='store_true')
parser.add_argument('--garg_crop',                             help='if set, crops according to Garg  ECCV16', action='store_true')
parser.add_argument('--eval_freq',                 type=int,   help='Online evaluation frequency in global steps', default=1)
...
if args.mode == 'train' and not args.checkpoint_path:
    ...
    model_name = 'bts_urban_v2_pytorch_att'
    import sys
...
eval_metrics = ['mse', 'badpix_7', 'badpix_3', 'badpix_1']

def compute_errors(gt, pred):

    train_diff = np.abs(pred - gt)
    mse = 100.0 * np.average(np.square(train_diff))
    badpix_7 = np.average((train_diff >= 0.07))
    badpix_3 = np.average((train_diff >= 0.03))
    badpix_1 = np.average((train_diff >= 0.01))

    return [mse, badpix_7, badpix_3, badpix_1]
...
def online_eval(model, dataloader_eval, gpu, ngpus,global_step, args):
    ...
    for i, eval_sample_batched in enumerate(tqdm(dataloader_eval.data)):
        with torch.no_grad():
            ...
            gt_depth = eval_sample_batched['valdata_label']
            scene = eval_sample_batched['scene'][0]
            
            B, N, C, H, W = image.shape
            merge_image = np.zeros([512, 512]).astype(float)
            for j in range(N):
                pred_depth = model(image[:, j, ...], args.att_rank)

                pred_depth = pred_depth.cpu().numpy().squeeze().astype(np.float32)
                # gt_depth = gt_depth[:, j, ...].cpu().numpy().squeeze().astype(np.float32)
                
                idx = (j // 4) * 128
                idy = (j % 4) * 128
                # if idx != 512-16 or idy!=512-16:
                merge_image[idx:idx + 128, idy:idy + 128] = pred_depth.copy()
            if args.rank % ngpus == 0:
                tmp = (merge_image - merge_image.min()) / (merge_image.max() - merge_image.min()) * 255.
                tmp_gt = gt_depth.cpu().numpy().squeeze().astype(np.float32)
                tmp_gt = (tmp_gt - tmp_gt.min()) / (tmp_gt.max() - tmp_gt.min()) * 255.
                cv2.imwrite('/data/yanzhengmao/projects/LFDepth/pytorch/output/' + scene + '_512x512.png', tmp)
                cv2.imwrite('/data/yanzhengmao/projects/LFDepth/pytorch/output/' + scene + '_512x512_gt.png', tmp_gt)
        ...
        if not args.multiprocessing_distributed or gpu == 0:
            ...
            print({'badpix_7': (1 - measures[1]) * 100})

        eval_measures[:4] += torch.tensor(measures).cuda(device=gpu)
        eval_measures[4] += 1
    ...
    if not args.multiprocessing_distributed or gpu == 0:
        ...
        print("{:>7}, {:>7}, {:>7}, {:>7}".format('mse', 'badpix_7', 'badpix_3', 'badpix_1'))
        for i in range(3):
            print('{:7.3f}, '.format(eval_measures_cpu[i]), end='')
        print('{:7.3f}'.format(eval_measures_cpu[3]))
        # if withwandb:
        #     wandb.log(
        #         {'mse': eval_measures_cpu[0], 'badpix_7': eval_measures_cpu[1], 'badpix_3': eval_measures_cpu[2], 'badpix_1': eval_measures_cpu[3]},
        #         step=global_step)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            depth_gt = torch.autograd.Variable(sample_batched['depth'].cuda(args.gpu, non_blocking=True)).unsqueeze(1)
            depth_est = model(image, args.att_rank)
            ...
            if global_step and global_step % args.log_freq == 0 and not model_just_loaded:
                ...
                print(print_string.format(args.gpu, examples_per_sec, loss, var_sum, var_sum/var_cnt, time_sofar, training_time_left))
            ...
            if not args.do_online_eval and global_step and global_step % args.save_freq == 0:
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    checkpoint = {'global_step': global_step,
                                  'model_flow_sence': model.state_dict(),
                                  'optimizer': optimizer.state_dict()}
                    torch.save(checkpoint, args.log_directory + '/' + args.model_name + '/model_flow_sence-{}'.format(global_step))
                ...
                if eval_measures is not None:
                    for i in range(4):
                        ...
                        if is_best:
                            ...
                            old_best_name = '/model_flow_sence-{}-best_{}_{:.5f}'.format(old_best_step, eval_metrics[i], old_best)
                            model_path = args.log_directory + '/' + args.model_name + old_best_name
                            if os.path.exists(model_path):
                                command = 'rm {}'.format(model_path)
                                os.system(command)
                            best_eval_steps[i] = global_step
                            model_save_name = '/model_flow_sence-{}-best_{}_{:.5f}'.format(global_step, eval_metrics[i], measure)
                            print('New best for {}. Saving model_flow_sence: {}'.format(eval_metrics[i], model_save_name))
                            checkpoint = {'global_step': global_step,
                                          'model_flow_sence': model.state_dict(),
                                          'optimizer': optimizer.state_dict(),
