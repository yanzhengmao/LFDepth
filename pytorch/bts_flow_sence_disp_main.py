...
from bts_dataloader_image import *
withwandb = False
try:
    import wandb
except ImportError:
...
# Log and save
parser.add_argument('--log_directory',             type=str,   help='directory to save checkpoints and summaries', default='./model_flow_sence_disp')
...
def compute_errors(gt, pred):
    ...
    bad_pixel_3 = (train_diff >= 0.03)
    bad_pixel_1 = (train_diff >= 0.01)

    mse = 100.0 * np.average(np.square(train_diff))
    badpix_7 = 100 - (100 * np.average(bad_pixel_7))
    badpix_3 = 100 -(100 * np.average(bad_pixel_3))
    badpix_1 = 100 - (100 * np.average(bad_pixel_1))
...
def online_eval(model, dataloader_test, gpu, ngpus, global_step, args):
    eval_measures = torch.zeros(10).cuda(device=gpu)
    for _, eval_sample_batched in enumerate(tqdm(dataloader_test.data)):
        with torch.no_grad():
            image = torch.autograd.Variable(eval_sample_batched['image'].cuda(gpu, non_blocking=True))
            depth_path = eval_sample_batched['image_path']
            # depth_gt = sample['depth']
            scenename = depth_path[0].split('/')[5]

            _, _, _, _, pred_depth = model(image, args.att_rank)
        ...
        measures = 100 - get_scores_file_by_name_clear(pred_depth, scenename)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.checkpoint_path != '':
        if os.path.isfile(args.checkpoint_path):
            ...
            model.load_state_dict(checkpoint['model_flow_sence_disp'])
    ...
    dataloader_test = BtsDataLoader(args, 'test')
            ...
            if not args.do_online_eval and global_step and global_step % args.save_freq == 0:
                if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0):
                    checkpoint = {'global_step': global_step,
                                  'model_flow_sence_disp': model.state_dict(),
                                  'optimizer': optimizer.state_dict()}
                    torch.save(checkpoint, args.log_directory + '/' + args.model_name + '/model_flow_sence_disp-{}'.format(global_step))

            if args.do_online_eval and global_step and global_step % args.eval_freq == 0 and not model_just_loaded:
                time.sleep(0.1)
                model.eval()
                eval_measures = online_eval(model, dataloader_test, gpu, ngpus_per_node,global_step, args)
                # if eval_measures is not None:
                #     for i in range(4):
                #         # eval_summary_writer.add_scalar(eval_metrics[i], eval_measures[i].cpu(), int(global_step))
                #         measure = eval_measures[i]
                #         is_best = False
                #         if i < 4 and measure < best_eval_measures_lower_better[i]:
                ...
                #             old_best_name = '/model_flow_sence_disp-{}-best_{}_{:.5f}'.format(old_best_step, eval_metrics[i], old_best)
                #             model_path = args.log_directory + '/' + args.model_name + old_best_name
                #             if os.path.exists(model_path):
                #                 command = 'rm {}'.format(model_path)
                #                 os.system(command)
                #             best_eval_steps[i] = global_step
                #             model_save_name = '/model_flow_sence_disp-{}-best_{}_{:.5f}'.format(global_step, eval_metrics[i], measure)
                #             print('New best for {}. Saving model_flow_sence_disp: {}'.format(eval_metrics[i], model_save_name))
                #             checkpoint = {'global_step': global_step,
                #                           'model_flow_sence_disp': model.state_dict(),
                #                           'optimizer': optimizer.state_dict(),
                ...
                #                           }
                model_save_name = '/model_flow_sence_disp_{}_{}_{:.5f}'.format(global_step, eval_metrics[0],
                                                                                    eval_measures[0])
                checkpoint = {'global_step': global_step,
                            'model_flow_sence_disp': model.state_dict(),
                            'optimizer': optimizer.state_dict()
