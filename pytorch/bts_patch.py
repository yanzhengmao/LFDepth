...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        # d = torch.log(depth_est[mask].float()+5.0) - torch.log(depth_gt[mask].float()+5.0)
        # d = abs(depth_est[mask] - depth_gt[mask])
        # return torch.sqrt((d ** 2).mean() - self.variance_focus * (d.mean() ** 2)) * 10.0

        # img_grad_x = depth_gt[:, :, :, :-1] - depth_gt[:, :, :, 1:]
        # img_grad_y = depth_gt[:, :, :-1, :] - depth_gt[:, :, 1:, :]
        # weight_x = torch.exp(-torch.abs(img_grad_x).mean(1).unsqueeze(1))
        # weight_y = torch.exp(-torch.abs(img_grad_y).mean(1).unsqueeze(1))
        #
        # loss = (((depth_est[:, :, :, :-1] - depth_est[:, :, :, 1:]).abs() * weight_x).sum() +
        #         ((depth_est[:, :, :-1, :] - depth_est[:, :, 1:, :]).abs() * weight_y).sum()) /\
        #         (weight_x.sum() + weight_y.sum())

        loss_L1 = torch.abs(depth_est.float() - depth_gt.float()).mean()
        #
        return loss_L1
