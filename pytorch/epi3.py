...
from TransUNet.networks.vit_seg_modeling_3epi_MLP_act import VisionTransformer as ViT_seg
from TransUNet.networks.vit_seg_modeling_3epi_MLP_act import CONFIGS as CONFIGS_ViT_seg
...
class get_loss(nn.Module):
    ...
    def forward(self, output,  pred_list, tra_hot_annotation):
...
class get_output_loss(nn.Module):
    def __init__(self):
        super(get_output_loss, self).__init__()
        self.logsoftmax = nn.LogSoftmax(dim=-1)
...
class BtsModel(nn.Module):
    ...
    def forward(self, x, focal):
        pred_list = []
        B, epi_nums, d, c, h, w = x.shape
        # x = x.view(B*epi_nums, d, c, h, w)
        x = x.view(B*epi_nums*d, c, h, w)
        # for i in range(4):
        #     skip_feat = self.encoder(x[:, i, ...], self.mask_ratio, B)
        #     pred_list.append(skip_feat)

        output = self.encoder(x, self.mask_ratio, B, d)
        # stack_4 = torch.stack(pred_list, dim=1)
