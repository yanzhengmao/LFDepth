...
# Training
parser.add_argument('--stage',                     type=int,   help='stage of training', default=1)
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    dataloader = BtsDataLoader(args, 'train', 128, '')
    global_step = 0
    ...
    epi_stage1_loss = get_stage1_loss()
    epi_stage2_loss = get_stage2_loss()
    epi_accuracy = get_accuracy()
    epi_mask = get_mask()
    epi_output_mask = get_output_mask()
    epi_mask_loss = get_mask_loss()
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            traindata_batch_0d = sample_batched['traindata_batch_0d'].cuda().float()
            traindata_batch_90d = sample_batched['traindata_batch_90d'].cuda().float()
            traindata_batch_45d = sample_batched['traindata_batch_45d'].cuda().float()
            traindata_batch_m45d = sample_batched['traindata_batch_m45d'].cuda().float()


            tra_annotations = sample_batched['tra_annotations'].cuda()
            tra_hot_annotation = sample_batched['tra_hot_annotation'].cuda()

            img = torch.stack([traindata_batch_0d, traindata_batch_90d, traindata_batch_45d, traindata_batch_m45d], dim=1)
            # img = img.permute(0,1,4,2,3)
            # traindata_label = traindata_label[:, :, 11:-11, 11:-11]
            # lpg8x8, lpg4x4, lpg2x2, reduc1x1, depth_est = model_Binocular([traindata_batch_90d, traindata_batch_0d, traindata_batch_45d, traindata_batch_m45d], args.att_rank)
            # with amp.autocast():
            # output, pred_list,_, _,_ = model(img, args.att_rank)
            output = model(img, args.att_rank)
            # if args.stage == 1:
            #     loss = epi_stage1_loss(output, pred_list, tra_hot_annotation)
            _, _, accuracy = epi_accuracy(output, tra_annotations)
            mask = epi_output_mask(output, tra_annotations, accuracy)
            loss = epi_mask_loss(output, tra_hot_annotation, mask)
            # loss = epi_stage1_loss(output, tra_hot_annotation)
            # else:
            #     mask = epi_mask(pred_list, tra_annotations)
            #     loss = epi_stage2_loss(output, pred_list, tra_hot_annotation, mask)
            ...
            # scaler.update()
            optimizer.step()
            lr_scheduler.step_update(epoch * steps_per_epoch + step)
            ...
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed and args.rank % ngpus_per_node == 0 and  global_step % args.log_freq == 0):
                print('[epoch][s/s_per_e/gs]: [{}][{}/{}/{}], lr: {:.12f}, loss: {:.12f}, train_score:{:.3f}'.format(epoch, global_step, steps_per_epoch, global_step, optimizer.param_groups[0]['lr'], loss, accuracy))
            # if step % 25000 == 0 and step != 0:
            #     for param_group in optimizer.param_groups:
            #         current_lr = param_group['lr'] * 0.8
            #         param_group['lr'] = current_lr
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed and global_step % args.save_freq == 0 and global_step):
                if args.rank % ngpus_per_node == 0:
                    model_save_name = '/epi_{}_{:5f}'.format(global_step, accuracy)
                    checkpoint = {'global_step': global_step,
                                'epi': model.state_dict(),
                                'optimizer': optimizer.state_dict()}
                    torch.save(checkpoint, args.log_directory + '/' + model_save_name)
            if not args.multiprocessing_distributed or (args.multiprocessing_distributed and global_step % args.eval_freq ==0 and global_step):
                if args.rank % ngpus_per_node == 0:
                    model.eval()
                    label_acc, scores_acc = online_test(model,args, epi_accuracy)
                    model_save_name = '/epi_{}_{:5f}_{:5f}'.format(global_step, label_acc, scores_acc)
                    checkpoint = {'global_step': global_step,
                                'epi': model.state_dict(),
                                'optimizer': optimizer.state_dict(),
...
def online_test(model,args, epi_accuracy):
    ...
    infer_list = []
    start = time.time()
    # center_view = misc.imread(('{}/{}/{}').format(img_root, imgs_list[idx], CENTER_VIEW_FNAME))
    ...
    scene_name = ['boxes', 'dots']
    # valid_hei = valid_wid = img_hei - 21 + 1
    ...
    print('average_score: {:5f}'.format((scores / 2)))
    print('average_disp_fuse_score: {:5f}'.format((disp_fuse_scores/2)))
    # print('average_max_of_4_score: {:5f}'.format((max_of_4_scores/2)))
    print('average_label_accuracy: {:5f}'.format((ava_label_acc / 2)))
    return ava_label_acc / 2, scores / 2
