...
class Embeddings(nn.Module):
    ...
    def __init__(self, config, img_size, in_channels=3):
        ...
        #     nn.Conv2d(2, config.hidden_size*4, kernel_size=1, stride=1, padding=0),
        #     nn.ReLU(),
    ...
    def forward(self, x):
        ...
        num_dirctions = b // batch_size
        x = x.view(batch_size, num_dirctions * n, -1)
    ...
    # def position_embedding(self, x, srcH, srcW):
    #     B, N, h, w = x.shape
    #     coords_h = torch.arange(0, h, device=x.device).float() * srcH / h
    #     coords_w = torch.arange(0, w, device=x.device).float() * srcW / w
    #     coords = torch.stack(torch.meshgrid([coords_h, coords_w])).unsqueeze(0).repeat(B, 1, 1, 1)
    #     coords = self.position_encoder(coords)
    #     coords = coords.squeeze().permute(0,2,1)
    #     return coords
