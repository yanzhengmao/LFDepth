...
# Log and save
parser.add_argument('--log_directory',             type=str,   help='directory to save checkpoints and summaries', default='./model_flow_sence')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    if args.checkpoint_path != '':
        if os.path.isfile(args.checkpoint_path):
            ...
            model.load_state_dict(checkpoint['model_flow_sence'])
    ...
    while epoch < args.num_epochs:
        ...
        for step, sample_batched in enumerate(dataloader.data):
            ...
            # image_r = torch.autograd.Variable(sample_batched['image_r'].cuda(args.gpu, non_blocking=True))
            # focal = torch.autograd.Variable(sample_batched['focal'].cuda(args.gpu, non_blocking=True))
            depth_gt = torch.autograd.Variable(sample_batched['depth'].cuda(args.gpu, non_blocking=True))
            # image = torch.cat((image,image_r),dim=1)
