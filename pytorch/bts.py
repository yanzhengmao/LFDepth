...
class silog_loss(nn.Module):
    ...
    def forward(self, depth_est, depth_gt, mask):
        ...
        loss_L1 = torch.abs(depth_est[mask].float() - depth_gt[mask].float()).mean()
        return loss_L1
...
class bts(nn.Module):
    ...
    def forward(self, features):
        ...
        final_depth = self.params.max_depth * 2 * self.get_depth(iconv1) - self.params.max_depth
...
class BtsModel(nn.Module):
    def __init__(self, params):
        ...
        self.encoder = ViT_seg(config_vit, img_size=[params.input_height,params.input_width], num_classes=config_vit.n_classes)
