...
class BtsDataLoader(object):
    def __init__(self, args, mode, input_size, scene_name):
        if mode == 'train':
            ...
            self.data = DataLoader(self.testing_samples, 246,
                                   shuffle=False,
                                   num_workers=1,
                                   pin_memory=True)
...
class DataLoadPreprocess(Dataset):
    def __init__(self, args, mode, input_size, scene_name, transform=None, is_for_online_eval=False):
        ...
        if self.mode == 'train':
            ...
            self.size = len(self.traindata_all) // len(self.sence)
        else:
            self.valdata_all, self.valdata_label = self.load_LFdata_test(scene_name)
            self.size = len(self.valdata_all) // len(self.test_sence)
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == 'train':
            ...
            if idx % self.size == 0:
                epi_patch_0_min1   = epi_patch_0
                epi_patch_90_min1  = epi_patch_90
                epi_patch_45_min1  = epi_patch_45
                epi_patch_135_min1 = epi_patch_135
            else:
                epi_patch_0_min1   = self.traindata_all[idx-1][0]
                epi_patch_90_min1  = self.traindata_all[idx-1][1]
                epi_patch_45_min1  = self.traindata_all[idx-1][2]
                epi_patch_135_min1 = self.traindata_all[idx-1][3]
            
            shuffle = idx // self.size
            if idx == self.size * (shuffle+1) - 1:
                epi_patch_0_plus1   = epi_patch_0
                epi_patch_90_plus1  = epi_patch_90
                epi_patch_45_plus1  = epi_patch_45
                epi_patch_135_plus1 = epi_patch_135
            else:
                epi_patch_0_plus1   = self.traindata_all[idx+1][0]
                epi_patch_90_plus1  = self.traindata_all[idx+1][1]
                epi_patch_45_plus1  = self.traindata_all[idx+1][2]
                epi_patch_135_plus1 = self.traindata_all[idx+1][3]

            epi_patch_90 = np.stack([epi_patch_90, epi_patch_90_min1, epi_patch_90_plus1], axis=0)
            epi_patch_0 = np.stack([epi_patch_0, epi_patch_0_min1, epi_patch_0_plus1], axis=0)
            epi_patch_45 = np.stack([epi_patch_45, epi_patch_45_min1, epi_patch_45_plus1], axis=0)
            epi_patch_135 = np.stack([epi_patch_135, epi_patch_135_min1, epi_patch_135_plus1], axis=0)
            ...
            epi_patch_0   = torch.from_numpy(epi_patch_0).permute(0, 3, 1, 2)
            epi_patch_90  = torch.from_numpy(epi_patch_90).permute(0, 3, 1, 2)
            epi_patch_45  = torch.from_numpy(epi_patch_45).permute(0, 3, 1, 2)
            epi_patch_135 = torch.from_numpy(epi_patch_135).permute(0, 3, 1, 2)
            ...
            # sample = {'traindata_batch_90d': torch.stack([epi_patch_90_min1, epi_patch_90, epi_patch_90_plus1],0),
            #           'traindata_batch_0d': torch.stack([epi_patch_0_min1, epi_patch_0, epi_patch_0_plus1],0),
            #           'traindata_batch_45d': torch.stack([epi_patch_45_min1, epi_patch_45, epi_patch_45_plus1],0),
            #           'traindata_batch_m45d': torch.stack([epi_patch_135_min1, epi_patch_135, epi_patch_135_plus1],0),
            #           'tra_annotations': tra_annotations,
            #           'tra_hot_annotation' : tra_hot_annotation}
            ...
            if idx % self.size == 0:
                ...
                epi_patch_0_min1   = self.valdata_all[idx-1][0]/ 255.0
                epi_patch_90_min1  = self.valdata_all[idx-1][1]/ 255.0
                epi_patch_45_min1  = self.valdata_all[idx-1][2]/ 255.0
                epi_patch_135_min1 = self.valdata_all[idx-1][3]/ 255.0
                epi_patch_0_min1 = R * epi_patch_0_min1[:,:,0] + G * epi_patch_0_min1[:,:,1] + B * epi_patch_0_min1[:,:,2]
                epi_patch_90_min1 = R * epi_patch_90_min1[:, :, 0] + G * epi_patch_90_min1[:, :, 1] + B * epi_patch_90_min1[:, :, 2]
                epi_patch_45_min1 = R * epi_patch_45_min1[:, :, 0] + G * epi_patch_45_min1[:, :, 1] + B * epi_patch_45_min1[:, :, 2]
                epi_patch_135_min1 = R * epi_patch_135_min1[:, :, 0] + G * epi_patch_135_min1[:, :, 1] + B * epi_patch_135_min1[:, :, 2]
            ...
            if idx == self.size * (shuffle+1) - 1:
                ...
                epi_patch_0_plus1   = self.valdata_all[idx+1][0]/ 255.0
                epi_patch_90_plus1  = self.valdata_all[idx+1][1]/ 255.0
                epi_patch_45_plus1  = self.valdata_all[idx+1][2]/ 255.0
                epi_patch_135_plus1 = self.valdata_all[idx+1][3]/ 255.0
                epi_patch_0_plus1 = R * epi_patch_0_plus1[:,:,0] + G * epi_patch_0_plus1[:,:,1] + B * epi_patch_0_plus1[:,:,2]
                epi_patch_90_plus1 = R * epi_patch_90_plus1[:, :, 0] + G * epi_patch_90_plus1[:, :, 1] + B * epi_patch_90_plus1[:, :, 2]
                epi_patch_45_plus1 = R * epi_patch_45_plus1[:, :, 0] + G * epi_patch_45_plus1[:, :, 1] + B * epi_patch_45_plus1[:, :, 2]
                epi_patch_135_plus1 = R * epi_patch_135_plus1[:, :, 0] + G * epi_patch_135_plus1[:, :, 1] + B * epi_patch_135_plus1[:, :, 2]
            ...
            epi_patch_0_min1 = torch.from_numpy(epi_patch_0_min1).unsqueeze(0)
            epi_patch_90_min1 = torch.from_numpy(epi_patch_90_min1).unsqueeze(0)
            epi_patch_45_min1 = torch.from_numpy(epi_patch_45_min1).unsqueeze(0)
            epi_patch_135_min1 = torch.from_numpy(epi_patch_135_min1).unsqueeze(0)

            epi_patch_0_plus1 = torch.from_numpy(epi_patch_0_plus1).unsqueeze(0)
            epi_patch_90_plus1 = torch.from_numpy(epi_patch_90_plus1).unsqueeze(0)
            epi_patch_45_plus1 = torch.from_numpy(epi_patch_45_plus1).unsqueeze(0)
            epi_patch_135_plus1 = torch.from_numpy(epi_patch_135_plus1).unsqueeze(0)

            sample = {'traindata_batch_90d': torch.stack([epi_patch_90,epi_patch_90_min1, epi_patch_90_plus1],0),
                      'traindata_batch_0d': torch.stack([epi_patch_0, epi_patch_0_min1, epi_patch_0_plus1],0),
                      'traindata_batch_45d': torch.stack([epi_patch_45, epi_patch_45_min1, epi_patch_45_plus1],0),
                      'traindata_batch_m45d': torch.stack([epi_patch_135, epi_patch_135_min1, epi_patch_135_plus1],0),
                      'tra_annotations': tra_annotations}
